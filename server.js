...
app.get('/overlay/:token', async (req, res) => {
    ...
    if (user) {
        ...
        if (user.overlay_theme === 'frost') fileToSend = 'overlay_frost.html';
    }
...
app.get('/stats-overlay/:token', async (req, res) => {
    ...
    if (user) {
        ...
        if (user.overlay_theme === 'basic') fileToSend = 'overlay_stats_basic.html';
        if (user.overlay_theme === 'frost') fileToSend = 'overlay_stats_frost.html';
    }
    res.sendFile(path.join(__dirname, fileToSend));
});
